 ...
 * not a permission for it — so the card still moves and only the disclosure
 * is stopped. That leaves the card near-blank in the current pool, which is
 * the honest reading rather than the flattering one.
 */
